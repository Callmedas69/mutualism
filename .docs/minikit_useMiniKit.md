...
<RequestExample>
  ...
  ```tsx components/ClientSpecificFeatures.tsx theme={null}
  import { useMiniKit } from '@coinbase/onchainkit/minikit';
  ...
  ```tsx components/AnalyticsTracker.tsx theme={null}
  import { useMiniKit } from '@coinbase/onchainkit/minikit';
  import { useEffect } from 'react';
...
```tsx components/FrameReady.tsx theme={null}
useEffect(() => {
  if (!isFrameReady) {
    setFrameReady();
  }
...
```tsx components/ClientDetection.tsx theme={null}
const isBaseApp = context.client.clientFid === '309857';
const isFarcaster = context.client.clientFid === '1';
